...
def check_job_compatibility(details_text: str) -> tuple[bool | None, str | None]:
    '''
    Checks the compatibility of the job based on LinkedIn's match indicators and skill alignment.
    Returns (should_apply, reason):
    - True, reason: Strong positive signal found (force apply)
    - False, reason: Strong negative signal found (skip)
    - None, None: No strong signal, fall back to standard filters
    '''
    if not details_text:
        return None, None
    ...
    # 1. Check for strong positive compatibility overrides
    positives = [
        "strong match",
        "top applicant",
        "high compatibility",
        ...
        "you match the required skills"
    ]
    for pos in positives:
        if pos in text_lower:
            return True, f"LinkedIn positive compatibility indicator found: '{pos}'"
            
    # 2. Check for explicit low relevance keywords
    negatives = [
        "low relevance",
        "poor alignment"
    ]
    for neg in negatives:
        if neg in text_lower:
            return False, f"LinkedIn negative compatibility indicator found: '{neg}'"
            
    # 3. Check for poor skill alignment using regex
    skills_match = re.search(r'(\d+)\s+of\s+(\d+)\s+skills?.*match', text_lower)
    if skills_match:
        matched_skills = int(skills_match.group(1))
        required_skills = int(skills_match.group(2))
        if required_skills > 0:
            match_percentage = (matched_skills / required_skills) * 100
            # Define "poor alignment" as less than 30% match rate,
            # or matching 1 or fewer skills when 4 or more are required.
            if match_percentage < 30.0 or (required_skills >= 4 and matched_skills <= 1):
                return False, f"Poor skill alignment: only {matched_skills} of {required_skills} skills match ({match_percentage:.1f}%)"
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    # Compatibility Check (Strong Match / Low Relevance / Skill alignment)
                    try:
                        details_pane = try_find_by_classes(driver, ["jobs-details__main-content", "jobs-search-two-pane__details", "jobs-details"])
                        details_text = details_pane.text if details_pane else ""
                        should_apply, comp_reason = check_job_compatibility(details_text)
                        if should_apply is True:
                            if skip:
                                print_lg(f"Override skip! LinkedIn indicated this job as a strong match. Reason: {comp_reason}")
                                skip = False
                        elif should_apply is False:
                            print_lg(f"Skipping job! {comp_reason}")
                            skip = True
                            reason = "Low relevance or poor skill alignment"
                            message = f"\n{description}\n\nSkipping due to: {comp_reason}\n"
                    except Exception as e:
                        print_lg("Failed during job compatibility check", e)
